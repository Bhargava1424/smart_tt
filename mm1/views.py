...
class Class:
    ...
    def set_meetingTime(self, meetingTime):
        if hasattr(meetingTime, 'time'):  # Ensure the MeetingTime object has the 'time' attribute
            # Assuming 'time' is the attribute of MeetingTime you wish to split
            time_str =meetingTime.day +' '+ meetingTime.time  # Get the string to split
            time_str = time_str.split(' ')[:-2] # Split the string and remove the last element
            self.meeting_time = time_str 
        else:
            # Handle the case where meetingTime does not have the 'time' attribute
            # You might want to set self.meeting_time to a default value or handle the error
            self.meeting_time = []

    # def set_meetingTime(self, meetingTime): self.meeting_time = meetingTime.split(' ')
